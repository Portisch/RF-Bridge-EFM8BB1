...
/*
 * Friedland Libra Doorbell, Issue #118
 */
#if EFM8BB1_SUPPORT_LIBRA_PROTOCOL == 1
#define LIBRA
SI_SEGMENT_VARIABLE(PROTOCOL_BUCKETS(LIBRA)[], static uint16_t, SI_SEG_CODE) = { 320, 620, 920, 1160 };
SI_SEGMENT_VARIABLE(PROTOCOL_START(LIBRA)[], static uint8_t, SI_SEG_CODE) = { HIGH(3), LOW(2) };
SI_SEGMENT_VARIABLE(PROTOCOL_BIT0(LIBRA)[], static uint8_t, SI_SEG_CODE) = { HIGH(0), LOW(1) };
SI_SEGMENT_VARIABLE(PROTOCOL_BIT1(LIBRA)[], static uint8_t, SI_SEG_CODE) = { HIGH(1), LOW(0) };
#endif
...
SI_SEGMENT_VARIABLE(PROTOCOL_DATA[], static struct BUCKET_PROTOCOL_DATA, SI_SEG_CODE) =
{
		 ...
		 */
		{
			{ &PROTOCOL_BUCKETS(LIBRA), ARRAY_LENGTH(PROTOCOL_BUCKETS(LIBRA)) },
			{ &PROTOCOL_START(LIBRA), ARRAY_LENGTH(PROTOCOL_START(LIBRA)) },
			{ &PROTOCOL_BIT0(LIBRA), ARRAY_LENGTH(PROTOCOL_BIT0(LIBRA)) },
			{ &PROTOCOL_BIT1(LIBRA), ARRAY_LENGTH(PROTOCOL_BIT1(LIBRA)) },
			{ NULL, 0 },
			34
		},
#endif
