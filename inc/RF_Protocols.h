...
/*
 * Idemo remote control, Issue #124
 */
#if EFM8BB1_SUPPORT_IDEMO_PROTOCOL == 1
#define IDEMO
SI_SEGMENT_VARIABLE(PROTOCOL_BUCKETS(IDEMO)[], static uint16_t, SI_SEG_CODE) = { 280, 630, 5030 };
SI_SEGMENT_VARIABLE(PROTOCOL_START(IDEMO)[], static uint8_t, SI_SEG_CODE) = { HIGH(2), LOW(1) };
SI_SEGMENT_VARIABLE(PROTOCOL_BIT0(IDEMO)[], static uint8_t, SI_SEG_CODE) = { HIGH(0), LOW(1) };
SI_SEGMENT_VARIABLE(PROTOCOL_BIT1(IDEMO)[], static uint8_t, SI_SEG_CODE) = { HIGH(1), LOW(0) };
#endif
...
SI_SEGMENT_VARIABLE(PROTOCOL_DATA[], static struct BUCKET_PROTOCOL_DATA, SI_SEG_CODE) =
{
		 ...
		 */
		{
			{ &PROTOCOL_BUCKETS(IDEMO), ARRAY_LENGTH(PROTOCOL_BUCKETS(IDEMO)) },
			{ &PROTOCOL_START(IDEMO), ARRAY_LENGTH(PROTOCOL_START(IDEMO)) },
			{ &PROTOCOL_BIT0(IDEMO), ARRAY_LENGTH(PROTOCOL_BIT0(IDEMO)) },
			{ &PROTOCOL_BIT1(IDEMO), ARRAY_LENGTH(PROTOCOL_BIT1(IDEMO)) },
			{ NULL, 0 },
			64
		},
#endif
