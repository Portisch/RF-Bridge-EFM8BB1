...
/*
 * AOK Remote
 * A-OK AC114-01B, AC123-01D,06D remote control for roller shades and motorized projector screens
 * All credits to - https://github.com/akirjavainen/A-OK
 */
#if EFM8BB1_SUPPORT_AOK_PROTOCOL == 1
#define AOK
SI_SEGMENT_VARIABLE(PROTOCOL_BUCKETS(AOK)[], static uint16_t, SI_SEG_CODE) = { 5300, 540, 270, 5030 };
SI_SEGMENT_VARIABLE(PROTOCOL_START(AOK)[], static uint8_t, SI_SEG_CODE) = { HIGH(0), LOW(1) };
SI_SEGMENT_VARIABLE(PROTOCOL_BIT0(AOK)[], static uint8_t, SI_SEG_CODE) = { HIGH(2), LOW(1) };
SI_SEGMENT_VARIABLE(PROTOCOL_BIT1(AOK)[], static uint8_t, SI_SEG_CODE) = { HIGH(1), LOW(2) };
SI_SEGMENT_VARIABLE(PROTOCOL_END(AOK)[], static uint8_t, SI_SEG_CODE) = { LOW(3) };
#endif
...
SI_SEGMENT_VARIABLE(PROTOCOL_DATA[], static struct BUCKET_PROTOCOL_DATA, SI_SEG_CODE) =
{
		 ...
		 */
		{
			{ &PROTOCOL_BUCKETS(AOK), ARRAY_LENGTH(PROTOCOL_BUCKETS(AOK)) },
			{ &PROTOCOL_START(AOK), ARRAY_LENGTH(PROTOCOL_START(AOK)) },
			{ &PROTOCOL_BIT0(AOK), ARRAY_LENGTH(PROTOCOL_BIT0(AOK)) },
			{ &PROTOCOL_BIT1(AOK), ARRAY_LENGTH(PROTOCOL_BIT1(AOK)) },
			{ &PROTOCOL_END(AOK), ARRAY_LENGTH(PROTOCOL_END(AOK)) },
			65
		},
#endif
};
